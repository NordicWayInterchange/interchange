...
public class MessageCollectorTest {
    ...
    @Test
    public void twoMatchesOneWithListenerEndpointWithDynamicFilterAndOneWithout() {
        Connection messageConnectionOne = mock(Connection.class);
        when(messageConnectionOne.canBeContacted(any())).thenReturn(true);
        ...
        ListenerEndpoint one = new ListenerEndpoint("one", "source-one", "endpoint-one", 5671,
                messageConnectionOne, "subscriptionExchange1", "originatingCountry='NO'");

        ListenerEndpoint two = new ListenerEndpoint("one", "source-two", "endpoint-two", 5671, messageConnectionTwo, "subscriptionExchange2");

        assertThat(one.getDynamicFilter()).isEqualTo("originatingCountry='NO'");
        assertThat(two.getDynamicFilter()).isEqualTo(null);
