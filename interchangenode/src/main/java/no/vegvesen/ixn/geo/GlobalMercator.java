...
public class GlobalMercator {
	...
	public String QuadTree( int tx, int ty, int zoom ) {
		...
		for( int i = zoom; i > 0; i-- ) { // Originally this was for( int i = zoom; i < 0; i-- ) {
			int digit = 0;
			int mask = 1 << (i - 1);
			if ((tx & mask) != 0) {
				digit += 1;
	...
	public String latLonToQuadTree(double lat, double lon, int zoom){
		...
		double[] latLonToMeters = LatLonToMeters(lat, lon);
		int[] metersToTile = MetersToTile(latLonToMeters[0], latLonToMeters[1], zoom);
		return QuadTree(metersToTile[0], metersToTile[1], zoom);
	}
