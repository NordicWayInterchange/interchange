...
public class NeighbourServiceDiscoveryTest {
	...
	public void gracefulBackoffPostOfCapabilityDoesNotHappenBeforeAllowedPostTime(){
		...
		ericsson.getControlConnectionBackoff().setBackoffStart(futureTime);
		ericsson.getControlConnectionBackoff().setConnectionStatus(ConnectionStatus.FAILED);
	...
	public void gracefulBackoffPollOfSubscriptionDoesNotHappenBeforeAllowedTime(){
		...
		ericsson.getControlConnectionBackoff().setBackoffStart(LocalDateTime.now().plusSeconds(10));
	...
	public void gracefulBackoffPostOfCapabilitiesHappensIfAllowedPostTimeHasPassed(){
		...
		ericsson.getControlConnectionBackoff().setBackoffStart(pastTime);
		doReturn(self).when(selfService).fetchSelf();
	...
	public void failedPostOfCapabilitiesInBackoffIncreasesNumberOfBackoffAttempts(){
		Neighbour ericsson = createNeighbour();
		ericsson.getControlConnectionBackoff().setBackoffAttempts(0);
		...
		assertThat(ericsson.getControlConnectionBackoff().getBackoffAttempts()).isEqualTo(1);
	}

	@Test
	public void unsuccessfulPostOfCapabilitiesToNodeWithTooManyPreviousBackoffAttemptsMarksCapabilitiesUnreachable(){
		Neighbour ericsson = createNeighbour();
		ericsson.getControlConnectionBackoff().setBackoffAttempts(4);
		...
		assertThat(ericsson.getControlConnectionBackoff().getConnectionStatus()).isEqualTo(ConnectionStatus.UNREACHABLE);
	...
	void unreachableNeighboursWillReceiveCapabilityExchangeWhenRetryingUnreachableNeighbours() {
		...
		n1.getControlConnectionBackoff().setConnectionStatus(ConnectionStatus.UNREACHABLE);
		n1.getControlConnectionBackoff().setBackoffStart(LocalDateTime.now().minusDays(2));

		Neighbour n2 = new Neighbour();
		n2.setName("neighbour-two");
		n2.getControlConnectionBackoff().setConnectionStatus(ConnectionStatus.UNREACHABLE);
		n2.getControlConnectionBackoff().setBackoffStart(LocalDateTime.now().minusDays(2));

		when(neighbourRepository.save(any(Neighbour.class))).thenAnswer(i -> i.getArguments()[0]); // return the argument sent in
		when(neighbourRepository.findByControlConnectionBackoff_ConnectionStatus(ConnectionStatus.UNREACHABLE)).thenReturn(Lists.list(n1, n2));
